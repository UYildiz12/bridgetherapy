...
| Homework sets | Therapist composes block documents (instructions, text answers, scales, choices, checklists, tables/logs, voice/drawing, app activities) with presets, one-click duplication, a live patient preview, and guarded Gemini draft assistance. Legacy item sets still render through adapters. |
| Homework assignment | Therapist assigns sets only to active linked patients; one-shot or recurring (daily/weekly entries until the due date). The patient list orders sets by what needs attention (revisions, overdue, today's entry) with progress bars, feedback chips, and daily streaks; the runner autosaves with flush-on-hide, shows part/entry progress, and reopens for editing when changes are requested. |
| Homework review | Therapist works a grouped review queue (needs review, awaiting changes, in progress, not started, reviewed) filterable by patient, then reviews per entry with completion markers, per-block comments, measure scores with a cross-entry trend, overall feedback, and revision requests. |
